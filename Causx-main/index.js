...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=55 74999250802 :+55 (74) 99925-0802\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calma ae porra to fazendo ⌛',
				success: '✔️ Sucesso ✔️',
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mancando os leke〙✪══\n╠➥'+teks+'╚═〘 CAUSS BOT 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 ATENÇÃO AQ」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
						...
						reply('Enviado pra todos com sucesso')
					}
					break
					case 'bcgc':
					client.updatePresence(from, Presence.composing) 
